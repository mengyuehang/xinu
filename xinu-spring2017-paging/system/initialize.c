...
frame_t *fifo_head;		/* head of mapped frame list: in time order */
uint32	gca_head;
...
static	void	sysinit()
{
	...
	freeframes = NFRAMES;
	gca_head = NFRAMES/2;
	initialize_frames(); // Initialize all necessary data structures.

	pd_t * null_pg_dir = get_directory(0);  // assign a page directory for null process
	...
	for (i = 0; i < NPROC; i++) {
		...
		prptr->prprio = 0;
		prptr->prpagedir = null_pg_dir;
		prptr->vp = 4096;		/* Virtual heap page number: 4096 */
		prptr->hsize = 0;	/* virtual heap size in pages */
		prptr->vlist = NULL;  /* Virtual heap list */
		prptr->vminaddr = 0;
		prptr->vmaxaddr = 0;
	}
...
static void initialize_paging()
{
	...
	prptr = &proctab[NULLPROC];
	initialize_global_pt(); // global page table: map pages 0-4095 to the 16MB physical addr. range
	initialize_device_pt(); // device page table: map the device memory starting at 0x90000000. (need to fill page directory entry at index #576 (0x90000000 >> 22) and
                            //all the entries in the corresponding page table)
	switch_page_directory((unsigned long)prptr->prpagedir); // set the PDBR register to the page direc. of NULLPROC
